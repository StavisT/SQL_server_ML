...
DROP PROCEDURE IF EXISTS [sp_train_adaboost_model]
GO

create procedure [dbo].[sp_train_adaboost_model] (@trained_model varbinary(max) OUTPUT)
as
begin
...
clf = AdaBoostRegressor(DecisionTreeRegressor(max_depth=6),n_estimators=500,random_state=42,loss="linear",learning_rate=0.01)
...
',
@input_data_1 = N'Select top 100000 cast(fare_amount as float) as fare_amount 
